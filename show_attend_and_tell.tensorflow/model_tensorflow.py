...
class Caption_Generator():
	...
	def build_model(self):
		...
		for ind in range(self.n_lstm_steps):
			...
			current_loss = tf.reduce_sum(cross_entropy,keepdims=True)
			loss = loss + current_loss

		loss = loss / tf.reduce_sum(mask,keepdims=True)
...
dim_hidden=256
ctx_shape=[196,512]
pretrained_model_path = '/media/dalek/Modus Operandi/Users/amit1/linux-files/model2/model-99'
#############################
###### 잡다한 Parameters #####
annotation_path = '/media/dalek/Modus Operandi/Users/amit1/linux-files/data/annotations.pickle'
feat_path = '/media/dalek/Modus Operandi/Users/amit1/linux-files/data/feats.npy'
model_path = '/media/dalek/Modus Operandi/Users/amit1/linux-files/model/'
#############################
...
def train(pretrained_model_path=pretrained_model_path): # 전에 학습하던게 있으면 초기값 설정.
	...
	with tf.Session() as sess:
		...
		for epoch in range(n_epochs):
			...
			epoch_cost = 0
			num_minibatches = int(len(captions)/batch_size)
...
def test(test_feat='./test_feats5.npy', model_path='/media/dalek/Modus Operandi/Users/amit1/linux-files/model2/model-99', maxlen=20):
	annotation_data = pd.read_pickle(annotation_path)
	captions = annotation_data['caption'].values
	wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)
	n_words = len(wordtoix)
	...
	return generated_sentence, alpha_list_val
